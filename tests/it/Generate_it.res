...
let setupConfig = (): unit => {
  ...
  | Exn.Error(err) => Js.Console.error(`Failed to setup config: ${Exn.message(err)->Option.getOr("Unknown")}`)
  }
}
...
let restoreConfig = (): unit => {
  ...
  | Exn.Error(err) => Js.Console.error(`Failed to restore config: ${Exn.message(err)->Option.getOr("Unknown")}`)
  }
}
